import { ConsumableItemRS, httpClient, PurchaseItemRQ } from '@/apis'
...
type Props = {
  record: ConsumableItemRS
  hideModal: () => void
}
const PurchaseModal = ({ record, hideModal }: Props) => {
  ...
  const getItemQuery = useQuery(['items', record.itemNo], () =>
    httpClient.items.getItem(record.itemNo)
  )

  const photoUrl = getItemQuery.data?.data?.photoUrl
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      // 신규 추가
      await httpClient.items.purchaseItem(record.itemNo, {
      ...
      <div className='flex items-center'>
        ...
        <div className='flex-1 ml-2'>
          <div className='flex items-center mb-4 ml-6 text-3xl font-bold'>
            ...
            <h1 className='flex-1 ml-3'>{record.name}</h1>
          </div>
          <Form
            form={form}
            name='basic'
